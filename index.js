	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: 'PRONTO AI ESTA'
				error: {
					stick: 'NÃO FOI POSSÍVEL FAZER ESSA FIGURINHA',
					Iv: 'LINK INVÁLIDO'
				},
				only: {
					group: '❌ VC ESTÁ USANDO O BOT NO PRIVADO❌',
					ownerG: 'VC NÃO É ADM OTÁRIO',
					ownerB: '❌ VC NÃO E MEU DONO❌',
					admin: 'VC NÃO É MEU DONO❌',
					Badmin: 'VC NÃO É ADM OTÁRIO'
				}
			}
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'setprefix':
					...
					prefix = args[0]
					reply(`O SINAL DO BOT FOI MUDANDO: ${prefix}`)
					break
				/*case 'loli':
				...
				case 'stalk':
					try {
						...
						console.log(`Error :`, color(e,'red'))
						 reply ('NOME DO USUÁRIO É INVÁLIDO')
					}
					break
			
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('O QUE VC QUER ESCREVE')
					teks = body.slice(7)
					reply(mess.wait)
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'clearall':
					if (!isOwner) return reply('ENTÃO É VC QUE ESTA USANDO MEU COMANDO') 
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('TODOS OS CHATS FOI APAGADO')
					break
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						teks = 'VC FOI PROMOVIDO PARABÉNS\n'
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'VC FOI REBAIXADO NÃO E MAIS ADM'\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('QUEM QUE VC QUER ADICIONAR') 
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('MARQUE ALGUEM PRA SER EXPULSO!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'OTARIO EXPULSO COM SUCESSO:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`OTARIO EXPULSO COM SUCESSO: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
//AQUI NAO PRECISA MUDAR		
...
//AQUI NAO PRECISA MUDAR                   
				...
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ MARQUE  A FIGURINHAS')
					reply(mess.wait)
...
//NAO FUNCIONA MAIS	
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('BEM VINDO A')
						welkom.push(from)
